        ...
        <div class="col-lg-12">
            ...
            <section class="panel">
                <header class="panel-heading">
                    {{ __(' ویرایش بلاگ ' . $blog->title ) }}
                </header>
                <div class="panel-body">
                    <div class="form">
                        <form  method="post" action="{{ route('blog.update' , $blog) }}" enctype="multipart/form-data" class="cmxform form-horizontal tasi-form" id="signupForm">
                            @csrf
                            @method('patch')
                            <div class="form-group ">
                                <label for="title" class="control-label col-lg-2">عنوان</label>
                                <div class="col-lg-10">
                                    <input id="title" name="title" type="text" value="{{ $blog->title }}" class="form-control @error('title') error @enderror" />
                                    @error('title')
                            ...
                            <div class="form-group ">
                                ...
                                <div class="col-lg-10">
                                    <input name="tags" id="tags" class="tagsinput" value="{{ $blog->tags }}" />
                                    @error('tags')
                                ...
                                <div class="col-lg-10">
                                    <select class="form-control m-bot15" name="category_id" id="category_id">
                                        @foreach($categories as $cat)
                                            <option value="{{ $cat->id }}" {{ $cat->id == $blog->category_id ? 'selected' : '' }}>{{ $cat->title }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group ">
                                ...
                                <div class="col-lg-10">
                                    ...
                                    <ck-editor :tag_name="{{  json_encode('body') }}" :value="{{ json_encode($blog->body)}}"></ck-editor>
